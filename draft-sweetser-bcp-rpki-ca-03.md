...
Flapping CA: A delegated CA that exhibits rapid or frequent changes in operational state, causing instability for relying party validators, analogous to route flapping in BGP.

Dead CA: A delegated CA that is persistently non-functional or unreachable for extended periods [@DEAD-CA].

Manifest: An RPKI signed object that lists the contents of a publication repository and provides integrity protection [@RFC9286], with manifest number handling further specified in [@RFC9981].
...
Dead CAs: Some delegated CAs become completely offline for extended periods (weeks to months) yet remain referenced in the RPKI hierarchy, causing validators to accumulate hundreds of thousands of failed synchronization attempts over time [@DEAD-CA]. (#dead-ca-management) defines detection criteria and remediation.

Flapping CAs: Certain delegated CAs exhibit intermittent availability that causes cache thrashing in RPKI validators and creates uncertainty about the current state of published objects. (#flapping-ca-detection-and-mitigation) defines detection patterns and mitigations.
...
### Infrastructure Requirements
   ...
   - CA operators **SHOULD** implement load balancing across publication points to distribute validator traffic, consistent with the multi-backend consistency requirements of [@I-D.ietf-sidrops-publication-server-bcp].
...
### Manifest Management
   ...
   - CA operators **SHOULD NOT** allow manifest gaps (periods without valid manifests) longer than 24 hours except during planned maintenance; registry operators **MAY** set a stricter maximum as an operational policy.
...
### Validation Requirements
   ...
   - CA operators **MUST** validate all RPKI objects before publication, including syntax, cryptographic signatures, and logical consistency. CRL Number extensions **SHOULD** be monotonically increasing and consistent with the corresponding manifest number, per [@RFC9829].
...
A CA **SHOULD** be considered persistently non-functional if:
   - Valid manifest and CRL cannot be retrieved for more than 60 days (equivalent to an Update Freshness value, as defined in [@I-D.fu-sidrops-rpki-repositories-monitoring], exceeding this threshold)
...
### Registry Operator Actions
   ...
   - Registry operators **SHOULD** revoke delegations for CAs that remain non-functional for more than 60-90 days after initial contact attempts; adopted regional practice varies within this range, for example 60 days under [@APNIC-PROP166] and 90 days under [@RIPE-2025-02].
...
### Performance Standards
   ...
   - Publication points **SHOULD** respond to HTTP requests within 10 seconds under normal conditions.
...
### Monitoring and Remediation
   ...
   - CA operators **MUST** monitor response times and error rates continuously; round-trip tracking of expected versus observed re-issued objects, as described in [@I-D.ietf-sidrops-publication-server-bcp], is a **RECOMMENDED** technique.
...
### Required Monitoring Metrics
   ...
   - Publication point availability and response time from multiple geographic locations, per the monitoring guidance in [@I-D.ietf-sidrops-publication-server-bcp]
...
CA operators **MUST** implement alerting for:
   ...
   - Manifest aging beyond an operator-defined threshold (commonly a few hours)
   - Certificate expiration warnings at multiple intervals before expiry (for example, 30, 7, and 1 days)
...
### Operational Disciplines:
   ...
   - CA operators **MUST** implement proactive certificate renewal procedures; initiating renewal at least 30 days before expiration is **RECOMMENDED**.
...
### Reporting Requirements
   ...
   - CA operators **MUST** report significant incidents or outages to registry operators promptly; within 24 hours is **RECOMMENDED**.
...
For planned CA retirement:
   ...
   - CA operators **MUST** provide advance notice to registry operators and affected communities; a minimum of 90 days is **RECOMMENDED**.
...
Registry operators **MUST** implement the monitoring capabilities described in (#registry-operator-monitoring), and **MUST** additionally establish clear Service Level Agreements (SLAs) and associated enforcement procedures for delegated CAs.
...
Recommended Tools:
   ...
   - Monitoring systems with RPKI-specific metrics and alerting, potentially built on a canonical cache representation such as [@I-D.ietf-sidrops-rpki-ccr] for audit and analytics pipelines
...
<reference anchor='APNIC-PROP166' target='https://www.apnic.net/community/policy/proposals/prop-166/'>
    <front>
        <title>prop-166: Revocation of Persistently Non-functional RPKI Certification Authorities</title>
        <author>
...
<reference anchor='RIPE-2025-02' target='https://www.ripe.net/community/policies/proposals/2025-02/'>
    <front>
        <title>2025-02: Revocation of Persistently Non-functional Delegated RPKI CAs</title>
        <author>
